/**
 * A template must not contradict itself or another about the same clause.
 *
 * The case this file was written for is gone: `cau_ban_btct` drew its own kerb
 * with a 10–40 cm range and a 25 cm default, while `go_chan_banh_tcvn` — the
 * standalone component for the same role — cited TCVN 11823-13:2017 điều 11.2
 * and refused anything outside 15–20 cm. The parent's *default* was already
 * outside the component's range, so the same office got a legal kerb or an
 * illegal one depending on which tool the assistant reached for, both claiming
 * the same standard. `cau_ban_btct` has since been withdrawn, so there is no
 * longer a second template drawing that kerb, and the paired check with it has
 * gone with it.
 *
 * What remains applies to every template there is, and is the half that would
 * have caught the parent on its own.
 */
